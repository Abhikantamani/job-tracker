...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 1 — DASHBOARD (original content, unchanged)
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    jobs = get_all_jobs()
...
with tab2:
    ...
    <div style="margin-bottom: 28px;">
        <div style="font-size: 1.35rem; font-weight: 700; color: #0a0a0a; letter-spacing: -0.03em;">
            Interview Prep Agent
            <span class="agent-tag">⚡ AGENTIC AI</span>
        </div>
        <div style="font-size: 0.85rem; color: #999; margin-top: 5px;">
            Paste a job description → Agent generates questions → You answer → Agent scores you
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if "agent_stage" not in st.session_state:
        st.session_state.agent_stage = "input"       # input → answering → results
    ...
    if st.session_state.agent_stage == "input":
        ...
        st.markdown('<div class="agent-step" style="animation-delay:0.1s">', unsafe_allow_html=True)
        st.markdown('<div class="agent-step-header">Step 1 of 3 — Job Details</div>', unsafe_allow_html=True)
        st.markdown('<div class="agent-step-title">Paste the Job Description</div>', unsafe_allow_html=True)

        role_input = st.text_input("Role / Position Title", placeholder="e.g. AI Engineer Intern")
        ...
        with col_btn:
            generate_btn = st.button("🔍 Analyse & Generate Questions")
        with col_hint:
            st.markdown(
                "<div style='padding-top:10px; font-size:0.8rem; color:#aaa;'>"
                "Agent will read the JD and generate 5 targeted interview questions.</div>",
                unsafe_allow_html=True
            )
        ...
        if generate_btn:
            if not jd_input.strip():
                ...
                # ── AGENT CALL 1: Generate Questions ─────────────────────────
...
Read this job description carefully and generate exactly 5 interview questions that are highly specific to this role.

Mix of question types:
- 2 technical/skills questions directly from the JD requirements
...
Return ONLY a numbered list of 5 questions. No preamble, no explanations. Just the questions.
Format:
1. [question]
2. [question]
3. [question]
...
5. [question]"""
    ...
    # ── STAGE 2: Answer Questions ─────────────────────────────────────────────
        ...
        st.markdown('<div class="agent-step" style="animation-delay:0.1s">', unsafe_allow_html=True)
        st.markdown('<div class="agent-step-header">Step 2 of 3 — Your Answers</div>', unsafe_allow_html=True)
        st.markdown(
        ...
        st.markdown(
            ...
            "The agent will evaluate every answer individually.</div>",
            unsafe_allow_html=True
        )
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        if submit_btn:
            ...
            if unanswered:
                st.warning(f"⚠️ Please answer all questions. Missing: Q{', Q'.join(map(str, unanswered))}")
            else:
                st.session_state.agent_answers = answers

                # ── AGENT CALL 2: Evaluate Each Answer ───────────────────────
                evaluations = []
                progress = st.progress(0, text="🤖 Agent evaluating your answers...")

                for i, question in enumerate(st.session_state.agent_questions, 1):
                    progress.progress(i / len(st.session_state.agent_questions),
...
TIP: [one quick tip to make this answer stronger]"""
                ...
                # ── AGENT CALL 3: Overall Readiness Score ────────────────────
                all_qa = "\n\n".join([
                    f"Q: {e['question']}\nA: {e['answer']}\nEval: {e['evaluation']}"
...
VERDICT: [one sentence — e.g. "Strong candidate with minor gaps in X"]
TOP_STRENGTH: [the candidate's single biggest strength shown in this interview]
CRITICAL_GAP: [the most important thing they need to work on]
NEXT_STEPS: [2-3 specific action items to improve before the real interview]"""
                    }]
                )
    ...
    # ── STAGE 3: Results ──────────────────────────────────────────────────────
        ...
        score_color = (
            "#166534" if overall_score >= 70
            else "#92400e" if overall_score >= 45
            else "#991b1b"
        )
        score_label = (
        ...
        <div class="overall-score-box" style="background: {score_color};">
            <div style="font-size:0.8rem; color:rgba(255,255,255,0.6); text-transform:uppercase;
                        letter-spacing:0.1em; margin-bottom:10px;">Overall Readiness Score</div>
            <div class="overall-score-number">{overall_score}<span style="font-size:2rem;">/100</span></div>
            <div style="font-size:1rem; font-weight:600; margin-top:10px; color:rgba(255,255,255,0.9);">
                {score_label}
            </div>
        </div>
        """, unsafe_allow_html=True)

        # Parse and show overall assessment
        overall_lines = {
        ...
        if overall_lines:
            cols = st.columns(3)
            fields = [
                ("TOP_STRENGTH", "💪 Top Strength", cols[0]),
                ("CRITICAL_GAP", "⚠️ Critical Gap", cols[1]),
                ("VERDICT",      "🏁 Verdict",      cols[2]),
            ]
                ...
                if val:
                    with col:
                        ...
                        <div class="eval-card" style="height:100%">
                            <div class="eval-question">{label}</div>
                            <div class="eval-feedback">{val}</div>
                        </div>""", unsafe_allow_html=True)

            next_steps = overall_lines.get("NEXT_STEPS", "")
            if next_steps:
                st.markdown(f"""
                <div class="eval-card" style="margin-top:4px;">
                    <div class="eval-question">📌 Next Steps Before the Interview</div>
                    <div class="eval-feedback">{next_steps}</div>
                </div>""", unsafe_allow_html=True)
        ...
        for i, item in enumerate(st.session_state.agent_evaluations, 1):
            ...
            badge_class = (
                "score-high" if q_score >= 7
                else "score-mid" if q_score >= 4
                else "score-low"
            )

            with st.expander(f"Q{i}. {item['question'][:80]}{'...' if len(item['question']) > 80 else ''}"):
                st.markdown(f"""
                <div class="eval-card">
                    <div class="eval-question">Your Answer</div>
                    <div class="eval-feedback" style="font-style:italic; color:#555;">"{item['answer']}"</div>
                    <span class="score-badge {badge_class}">{q_score}/10</span>
                </div>
                """, unsafe_allow_html=True)

                row1, row2, row3 = st.columns(3)
                for col, key, label in [
                    (row1, "STRENGTH",    "✅ Strength"),
                    (row2, "IMPROVEMENT", "🔧 Improvement"),
                    (row3, "TIP",         "💡 Quick Tip"),
                ]:
